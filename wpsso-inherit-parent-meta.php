	...
	class WpssoIpm {
		...
		public static function show_required_notices() {
			...
			foreach ( $info[ 'req' ] as $ext => $req_info ) {
				...
				$notice_msg = __( 'The %1$s add-on requires the %2$s plugin &mdash; install and activate the plugin or <a href="%3$s">deactivate this add-on</a>.', 'wpsso-inherit-parent-meta' );
		...
		public static function check_wp_version() {
			...
			if ( version_compare( $wp_version, self::$wp_min_version, '<' ) ) {
				...
				wp_die( '<p>' . sprintf( __( '%1$s requires %2$s version %3$s or higher and has been deactivated.',
					'wpsso-inherit-parent-meta' ), $plugin_data[ 'Name' ], 'WordPress', self::$wp_min_version ) . ' ' . 
		...
		private function min_version_notice() {
			...
			if ( is_admin() ) {
				...
				$notice_msg = sprintf( __( 'The %1$s version %2$s add-on requires %3$s version %4$s or newer (version %5$s is currently installed).',
					'wpsso-inherit-parent-meta' ), $info[ 'name' ], $info[ 'version' ], $req_info[ 'name' ], $req_info[ 'min_version' ],
