	...
	class WpssoIpm {
		...
		public static function check_wp_min_version() {
			...
			if ( version_compare( $wp_version, self::$wp_min_version, '<' ) ) {
				...
				$notice_version_transl = __( '%1$s requires %2$s version %3$s or newer and has been deactivated.', 'wpsso-inherit-parent-meta' );

				$notice_upgrade_transl = __( 'Please upgrade %1$s before trying to re-activate the %2$s plugin.', 'wpsso-inherit-parent-meta' );
