...
if ( ! class_exists( 'WpssoIpm' ) ) {
	...
	class WpssoIpm {
		 ...
		 */
		private static $ext            = 'wpssoipm';
		private static $p_ext          = 'ipm';
		...
		private static function get_missing_requirements() {
			...
			$info = WpssoIpmConfig::$cf[ 'plugin' ][ self::$ext ];
			...
			foreach ( $info[ 'req' ] as $key => $req_info ) {
				...
				if ( ! empty( $req_info[ 'version_global' ] ) && ! empty( $GLOBALS[ $req_info[ 'version_global' ] ] ) ) {
					...
					$notice_msg = __( 'The %1$s version %2$s add-on requires the %3$s plugin &mdash; please activate the missing plugin.', 'wpsso-inherit-parent-meta' );
				...
				if ( ! empty( $req_info[ 'version' ] ) ) {
					...
					if ( ! empty( $req_info[ 'min_version' ] ) ) {
						...
						if ( version_compare( $req_info[ 'version' ], $req_info[ 'min_version' ], '<' ) ) {
							...
							$notice_msg = __( 'The %1$s version %2$s add-on requires %3$s version %4$s or newer (version %5$s is currently installed).', 'wpsso-inherit-parent-meta' );
